...
#finding best models on validation and test sets
...
print('Ridge best MSE on validation set: %g, corresponding MSE on test set:%g' % (MSE_cv_val_Ridge[min_ind_cv_val_Ridge[0],min_ind_cv_val_Ridge[1]], MSE_cv_val_Ridge[min_ind_cv_test_Ridge[0],min_ind_cv_test_Ridge[1]]))

#plot Franke/terrain estimate for best Ridge model
...
plot_several(np.repeat(poly_degrees[:,None], 3, axis=1), np.hstack((MSE_bootstrap_test_Ridge[:,0][:,None],bias_bootstrap_test_Ridge[:,0][:,None],variance_bootstrap_test_Ridge[:,0][:,None])), ['r-', 'b-', 'g-'], ['MSE', 'bias^2', 'variance'], 'Polynomial degree', '', 'Bias-variance trade-off (Bootstrap, Ridge, \u03BB = 10^(-7))', savefig = savefigs, figname = 'bias_var_bootstrap_Ridge' + fignamePostFix)
...
#show parameters for Lasso polynomial degree 5, dependence on lambda
beta_Lasso_d5 = np.squeeze(np.asarray(beta_Lasso[4][:]))
heatmap(beta_Lasso_d5.T, 'Lasso parameters size dependence on \u03BB (degree 5)', '\u03BB', 'parameters', lambda_tests_str, list_of_features[4], True, savefig = savefigs, figname = 'LassoParamSize' + fignamePostFix)
